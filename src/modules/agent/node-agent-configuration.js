...
const FIELDS = ["agent_id", "agent_name", "gateway_url", "credential_ref", "enabled", "status", "created_at", "updated_at"];
...
function project(profile) {
  ...
  return validateConfiguration(Object.fromEntries(FIELDS.map((field) => [field, profile[field]])));
}

function validateConfiguration(value) {
  if (!value || typeof value !== "object" || Object.keys(value).length !== FIELDS.length || FIELDS.some((field) => value[field] === undefined)
