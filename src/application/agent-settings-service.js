...
export function createAgentSettingsService({ profiles, configuration, gateway, now = () => new Date().toISOString(), secretStore = new Map() } = {}) {
  ...
  function defaults(agentId, agentName) { return { agent_id: agentId, agent_name: agentName, gateway_url: "https://gateway.example.test/agent", credential_ref: `runtime:${agentId}:api-key`, enabled: false, status: "disabled", created_at: null, updated_at: null }; }
}
