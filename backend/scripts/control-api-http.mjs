...
export function createControlApiHttp({ services } = {}) {
  const { bus, communications, conversations, eventStore, indexDb, subscriptions, knowledge, roadmaps, sprintPlans, provenance, relevantTreeSelector, decisions, agentSettings, sprintPlanUpload, sprintOrchestration, dispatchSprint, dispatchTicket, runToolLab, internalBus, ticketCrudService, buildBuilderContext, protocolStorage, agentGateway, agentConfiguration, sdkGateways, conversationStateStore, fileService, projectRoot, publishUnifiedStreamEvent, logEvent, projectId } = services;
  const agentLoopLogger = createRuntimeLogger({ logEvent, source: "owner-chat-agent-loop" });
  const sdkStream = createOwnerSdkStream({ agentConfiguration, sdkGateways: { ...sdkGateways, anthropic: createAnthropicConversationGateway({ agentGateway, projectLogger: agentLoopLogger.emit }) }, fallbackStream: (input) => agentGateway.stream(input), conversationStateStore, conversationMessages: communications, fileService, projectRoot, projectLogger: agentLoopLogger.emit });
