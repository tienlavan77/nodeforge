...
async function start() {
  ...
  const nodePid = launch(process.execPath, ["backend/scripts/start-dev.mjs"], join(logDir, "node.log"));
  ...
  console.log("  [CLIENT]  Next.js Web UI — run separately: pnpm --dir ui/nextjs dev");
}
...
async function stop() {
  ...
  console.log("  [NOT TOUCHED] Next.js Web UI (client process)");
