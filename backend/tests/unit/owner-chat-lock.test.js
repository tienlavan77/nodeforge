...
function setup() {
  const internalBus = new EventEmitter(); const sent = [];
  const bus = { send: (value) => { sent.push(value); return value; }, sendFast: () => {}, flush: async () => {} };
  const service = createOwnerChatService({ bus, internalBus });
  return { internalBus, sent, service };
}

test("rejects input while running and reopens on done", async () => {
  const { internalBus, sent, service } = setup();
  service.submit(input("M1", "hello"));
  ...
  assert.equal(rejected.message_type, "ticket.input_rejected");
...
test("race between status and send rejects chat while locked", () => {
  const { internalBus, service } = setup();
  internalBus.emit("node.status_change", { task_id: "T", payload: { conversation_id: "CONV-BU-P-T", to: "running" } });
  const result = service.submit(input("M-RACE", "hello"));
  assert.equal(result.message_type, "ticket.input_rejected");
});
