...
const TABLES = ["calls", "dependency_edges", "file_content_fts", "file_content_fts_config", "file_content_fts_content", "file_content_fts_data", "file_content_fts_docsize", "file_content_fts_idx", "files", "imports_exports", "index_metadata", "references", "symbol_content_fts", "symbol_content_fts_config", "symbol_content_fts_content", "symbol_content_fts_data", "symbol_content_fts_docsize", "symbol_content_fts_idx", "symbols", "tests_map"];
...
test("creates the runtime directory and migrates index.db exactly once", async () => {
  ...
  try {
    ...
    assert.deepEqual(firstOpen.all("SELECT version FROM schema_migrations ORDER BY version"), [{ version: 1 }, { version: 2 }, { version: 3 }, { version: 4 }, { version: 5 }]);
    ...
    assert.deepEqual(secondOpen.all("SELECT version FROM schema_migrations ORDER BY version"), [{ version: 1 }, { version: 2 }, { version: 3 }, { version: 4 }, { version: 5 }]);
