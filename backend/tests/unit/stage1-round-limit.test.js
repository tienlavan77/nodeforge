...
test("round limit escalates to needs_human_review without calling provider again", async () => {
  ...
  const runner = createStage1TicketRunner({ maxRounds: 1, statusStore, gitService: { branchExists: async () => false, createBranch: async () => {}, commit: async () => ({}) }, protocolLogger: { requestSent() {}, responseReceived() {}, failed() {} }, fileService: { readFile: async () => "", atomicWrite: async () => {}, atomicCreate: async () => {} }, files: { findByPath: (path) => path === "src/app.jsx" ? { file_id: "F1", path, language: "javascript" } : null }, requestBuilder: createStage1TaskRequestBuilder(), agentGateway: { request: async () => { calls += 1; return { payload: { tool_use: { name: "request_info", input: { files_requested: ["src/app.jsx"], reason: "Need source" } } } }; } } });
