...
test("routes completed and continue status responses", async () => {
  const router = createStage1ResponseRouter({ onCodeNeeded: () => {}, onSubmitCode: () => {}, onCompleted: (e) => e.payload.report, onContinue: (e) => e.payload.next_task });
  assert.deepEqual(await router.routeResponse({ ...base, type: "completed", payload: { report: { summary: "done" } } }), { summary: "done" });
  assert.deepEqual(await router.routeResponse({ ...base, type: "continue", payload: { next_task: { description: "Wire page" } } }), { description: "Wire page" });
});
