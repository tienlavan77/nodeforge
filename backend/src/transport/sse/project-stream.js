...
const PROJECT_EVENT_TYPES = new Set([...INDEX_EVENT_TYPES, "ticket.created", "ticket.updated", "ticket.status_change", "ticket.status_changed", "ticket.deleted", "ticket.creation", "sprint.created", "sprint.updated", "sprint.deleted"]);

/** Project-scoped SSE projection for the initial stream contract. */
export function createProjectStream({ projectId, indexDb, watcherSnapshot, subscriptions, eventBus, heartbeatMs = 15000, clock = () => new Date().toISOString() } = {}) {
