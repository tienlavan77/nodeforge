...
export function normalizeResponse(rawResponse, requestContext = {}) {
  ...
  if (type === "submit_code_response") payload = canonicalizeSubmitCodePayload(payload);
  if (!type) throw providerError("PROVIDER_TOOL_UNSUPPORTED", `Unsupported OpenAI tool: ${tool.name || "<unknown>"}.`);
  const envelope = {
...
// Normalizes submit_code_response payloads without accepting legacy transport fields.
function canonicalizeSubmitCodePayload(payload) {
