  ...
  async function execute({ agent, agentId, prompt, correlationId, options = {} } = {}) {
    ...
    try {
      const agentOptions = {
        ...
        tools: (options.forgeTools?.definitions ?? []).filter((definition) => typeof options.forgeTools.registry?.[definition.name]?.execute === "function")
          .map((definition) => tool({ name: definition.name, description: definition.description,
            // Forge schemas have optional fields that this gateway rejects after the SDK rewrites them for strict mode.
            // Node still authorizes and validates every tool call before executing it.
            parameters: definition.input_schema, strict: false,
            execute: (input) => options.forgeTools.registry[definition.name].execute(input, options.forgeTools.context) }))
      };
      ...
      const result = await runner(openaiAgent, prompt, { modelProvider: provider, signal: controller.signal, maxTurns: agentOptions.tools.length ? 6 : 1, tracingDisabled: true });
      return {
