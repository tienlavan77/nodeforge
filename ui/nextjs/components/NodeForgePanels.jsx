...
function TicketModal({ ticket, client, projectId, onRefreshed, onClose }) {
  ...
  return <EntityDetailsModal title={ticket.id} modalClassName="ticket-language-modal" onClose={onClose}><div className="ticket-language-summary"><p className="sprint-objective">{generatedFields.title || ticket.title}</p><p><strong>Status:</strong> {ticket.status} · {ticket.progress}%</p><dl className="ticket-generated-fields"><dt>Objective</dt><dd>{generatedFields.objective || "—"}</dd><dt>Acceptance criteria</dt><dd>{generatedFields.acceptance_criteria.length ? <ul>{generatedFields.acceptance_criteria.map((item, index) => <li key={`${index}-${item}`}>{item}</li>)}</ul> : "—"}</dd></dl></div><form className="ticket-content-change-panel" onSubmit={submitContentChange}><div className="ticket-language-header"><div><h3>Vietnamese context</h3><p>Provide Vietnamese context to generate the English ticket content.</p></div></div><label htmlFor={`ticket-vietnamese-context-${ticket.id}`}>Vietnamese context<textarea id={`ticket-vietnamese-context-${ticket.id}`} value={vietnameseContext} onChange={(event) => setVietnameseContext(event.target.value)} rows={8} placeholder="Enter Vietnamese context..." /></label><div className="ticket-regenerate-actions"><button className="ticket-regenerate-button" type="submit" disabled={!hasVietnameseContextEdit || submitState === "submitting"}>{submitState === "submitting" ? "Generating…" : "Generate English"}</button></div>{error && <p className="dashboard-state error" role="alert">{error}</p>}{submitState === "done" && !error && <p className="dashboard-state" role="status">English ticket generated.</p>}<div className="ticket-regenerated-content"><h3>English ticket</h3><p>{englishContent || "No English content generated yet."}</p></div></form></EntityDetailsModal></EntityDetailsModal>;
}
