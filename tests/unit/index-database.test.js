...
const TABLES = ["calls", "dependency_edges", "files", "imports_exports", "references", "symbols", "tests_map"];
...
test("creates the runtime directory and migrates index.db exactly once", async () => {
  ...
  try {
    ...
    assert.deepEqual(firstOpen.all("SELECT version FROM schema_migrations ORDER BY version"), [{ version: 1 }, { version: 2 }, { version: 3 }]);
    ...
    assert.deepEqual(secondOpen.all("SELECT version FROM schema_migrations ORDER BY version"), [{ version: 1 }, { version: 2 }, { version: 3 }]);
