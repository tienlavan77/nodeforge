...
test("persists ordered real Agent stream deltas and completion for SSE replay", async () => {
  ...
  assert.equal(messages[2].payload.text, "one");
  assert.equal(messages[3].payload.text, " two");
...
test("forwards the first delta immediately for short responses", async () => {
  ...
  await new Promise((resolve) => setImmediate(resolve));
  const messages = store.getByConversationId("CONV-149-SHORT");
  assert.equal(messages.find((item) => item.message_type === "architecture.message.delta").payload.text, "short");
});
